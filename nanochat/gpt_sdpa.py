"""
GPT model using PyTorch native SDPA (for Ampere GPUs that don't support FA3).
Drop-in replacement for gpt.py — same GPTConfig, same interface.
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, cos_sin, window_size, kv_cache):
        ...
        q = norm(q.transpose(1, 2)).transpose(1, 2)
        k = norm(k.transpose(1, 2)).transpose(1, 2)
        ...
        # Use SDPA with causal=True (no sliding window for simplicity on baseline)
